...
* ![Table look](https://gisworks.ru/qgis_tools/img/line_favorites.png) - список "избранного"
...
>[!NOTE]
> Начиная с версии 1.6, для набора скриптов используются два домена: [gisworks.ru](https://gisworks.ru/) и [gisworks.org](https://gisworks.org/).
> Оба сайта предоставляют одинаковый набор скриптов. Однако первый предназначен для пользователей из России, испытывающих трудности с доступом к европейским и американским хостингам, а второй — для тех, у кого нет доступа к российским хостингам.
> Домен можно изменить вручную в *Настройках плагина*.
...
- флажок избранного: добавляет кнопку `Избранное`, с помощью которой можно получать список избранных инструментов 
- флажок функций для разработчиков: добавляет дополнительные опции для списка скриптов
...
![Table loook](https://gisworks.ru/qgis_tools/img/kolba_settings.png?new=true16)
...
**Favorites enabled** добавляет и убирает кнопку `Favorites` (избранное), нажатие по которой позволяет увидеть список инструментов, помеченных пользователем как избранные. Чтобы сделать инструмент избранным, необходимо нажать правой кнопкой мыши по любому скрипту и выбрать `Add to favorites`. После этого если кликнуть по кнопке `Favorites`, появится список избранных скриптов. Чтобы убрать скрипт из избранного, необходимо, находясь в этом меню, также кликнуть правой кнопкой мыши по скрипту и выбрать `Remove from favorites`.

**Developers options** позволяют перетаскивать скрипты в редактор консоли Python в QGIS или в любой внешний текстовый редактор (например, Visual Studio Code, PyCharm, Sublime). Это может быть полезно разработчикам, которые не хотят тратить время на поиск и открывание скрипта через проводник. Кроме того, данная настройка добавляет для скриптов две опции: `Open in Python console` и `Reveal in file explorer`. Первая загружает скрипт в редактор консоли Python в QGIS, а вторая открывает папку, содержащую файл скрипта, и выделяет этот файл.
...
<details>
  ...
  ### v1.5
  - закладки
  - пользовательский набор скриптов в WebScripts
  - импорт/экспорт конфигурации настройки плагина
  - поиск скриптов в WebScripts без учёта регистра
